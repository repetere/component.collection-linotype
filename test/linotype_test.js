...
describe('Linotype - paige compositor', function () {
	...
	describe('Customization and overwriteable options', function () {
		...
		it('should allow you to overwrite default options',function(){
			linotypeTest = new this.browser.window.linotype({resize:false});
		...
		it('should throw ConfigurationError for incompatible scrolling options',function(){
			try{
				expect(function(){
					linotypeTest = new this.browser.window.linotype({continuousVertical:true,loopBottom:true});
				}).to.throw(Error);
		...
		it('Should allow you to override default scroll Delay',function(){
			linotypeTest = new this.browser.window.linotype({scrollDelay:900});
		...
		it('Should throw RangeError if scroll delay is less than 400',function(){
			try{
				expect(function(){
					linotypeTest = new this.browser.window.linotype({scrollDelay:399});
				}).to.throw(Error);
